    ...
    <div class="row">
        <div class="col-12">
            ...
            <div class="card card_border">
                <div class="card-header card_header_color lang" key="unassign_charge">Additional Charge</div>
            ...
            <div class="card card_border">
                <div class="card-header card_header_color lang" key="unassignChargedetails"></div>
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/xlsx/0.18.5/xlsx.full.min.js"></script>
    <script>
        ...
        function getroletypecode_basedondept(deptcode, roletypecode) {
            ...
            const defaultOption = `<option value="" data-name-en="Select Role Type" data-name-ta="களத்தணிக்கை பங்கு நிலையை தேர்ந்தெடுக்கவும்">
    ...
    const $dropdown = $('#userid'); // Dropdown for users
    const deptcode = $('#deptcode').val();


    // Ensure the values are valid for the given conditions
    if ((roletypecode) && 
        ((roletypecode == '<?php echo $Ho_roletypecode; ?>' && deptcode) ||
        (roletypecode == '<?php echo $Re_roletypecode; ?>' && deptcode && regioncode && instmappingcode) ||
        (roletypecode == '<?php echo $Dist_roletypecode; ?>' && deptcode && regioncode && distcode && instmappingcode)) &&
		...
		deptcode    :   deptcode,
            ...
            success: function(response) {
                const lang = getLanguage(); // Assuming this function gets the user's language
                ...
                // Update select color (ensure `updateSelectColorByValue` function is available)
                updateSelectColorByValue(document.querySelectorAll(".form-select"));
            },
            error: function(xhr) {
                console.error("Error during AJAX request:", xhr);

                // Show error message and reset dropdown in case of error
                $dropdown.html('<option value="">Error loading data</option>');
                passing_alert_value(
                    'Alert',
                    'Error loading data. Please try again.',
        ...
        // If the conditions are not met, show an alert with a custom message
        passing_alert_value(
            'Alert',
...
function extractIfhrms(html) {
    ...
    return match ? match[1] : "-";
        ...
        jsonLoadedPromise.then(() => {
            ...
            function unassign_insertupdate(action)
            {
                $.ajaxSetup({
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                ...
                $.ajax({
                    url: '/unassigncharge_insertupdate', // URL where the form data will be posted
                    type: 'POST',
                    data: formData,
                    success: function(response) 
                    {
                        if (response.success) 
                        {
                            if(action == 'check')
                            {
                                ...
                                $('#process_button').off('click').on('click', function(event) {
                                    ...
                                    unassign_insertupdate('unassign')
                                });
                                passing_alert_value('Confirmation', 'Are you sure to unassign the charge?', 'confirmation_alert', 'alert_header',
