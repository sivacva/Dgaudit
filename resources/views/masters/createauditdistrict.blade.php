...
<div class="row">
    <div class="col-12">
        <div class="card card_border">
            <div class="card-header card_header_color lang" key="auditDistrict">Audit District</div>
            <div class="card-body">
                <form id="auditdistrictform" name="auditdistrictform">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 mb-3">
                            <label class="form-label required lang " key="auditdistename" for="auditdistename">Audit
                                District English Name</label>
                            <input type="text" class="form-control name" id="auditdistename" name="auditdistename"
                                maxlength="50" data-placeholder-key="auditdistename" required>
                        </div>

                        <div class="col-md-4 mb-3">
                            <label class="form-label required lang " key="auditdisttname" for="auditdisttname">Audit
                                District Tamil Name</label>
                            <input type="text" class="form-control name" id="auditdisttname" name="auditdisttname"
                                maxlength="50" data-placeholder-key="auditdisttname" required>
                        </div>

                        <div class="col-md-4 mb-3">
                            <label class="form-label required lang name" key="auditdistsname" for="auditdistsname">Audit
                                District Short Name Name</label>
                            <input type="text" class="form-control name" id="auditdistsname" name="auditdistsname"
                                maxlength="3" data-placeholder-key="auditdistsname" required>
                        </div>
                    ...
                    <div class="row ">
                        <div class="col-md-3 mx-auto text-center">
                            ...
                            <input type="hidden" name="auditdistid" id="auditdistid" value="" />

                            <input type="hidden" name="auditdistcode" id="auditdistcode" value="" />
        ...
        <div class="card card_border">
            <div class="card-header card_header_color lang" key="auditDistricttable">Audit District Details</div>
            <div class="card-body"><br>
                <div class="datatables">
                    <div class="table-responsive hide_this" id="tableshow">
                        <table id="auditdistricttable"
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th class="lang align-middle text-center" key="auditdistename">Audit District
                                        English Name</th>
                                    <th class="lang align-middle text-center" key="auditdisttname">Audit District
                                        Tamil Name</th>
...
<script>
    ...
    function initializeDataTable(language) {
        $.ajax({
            url: "{{ route('auditdistrict.auditdistrict_fetchData') }}",
            type: "POST",
            headers: {
    ...
    function renderTable(language) {
        ...
        if ($.fn.DataTable.isDataTable('#auditdistricttable')) {
            $('#auditdistricttable').DataTable().clear().destroy();
        }

        table = $('#auditdistricttable').DataTable({
            "processing": true,
            "serverSide": false,
            "lengthChange": false,
            "data": dataFromServer,
            columns: [{
                ...
                {
                    data: "auditdistename",
                    title: columnLabels?.["auditdistename"]?.[language],
                    className: "d-none d-md-table-cell lang extra-column text-wrap",
                    render: function(data, type, row) {
                        return row.auditdistename || '-';
                    }
                },
                {
                    data: "auditdisttname",
                    title: columnLabels?.["auditdisttname"]?.[language],
                    className: "d-none d-md-table-cell lang extra-column text-wrap",
                    render: function(data, type, row) {
                        return row.auditdisttname || '-';
                    }
                },
                {
                    data: "statusflag",
            ...
            "initComplete": function(settings, json) {
                $("#auditdistricttable").wrap(
                    "<div style='overflow:auto; width:100%;position:relative;'></div>");
            },

        });
        const mobileColumns = ["auditdistename", "auditdistename", "statusflag"];
        setupMobileRowToggle(mobileColumns);

        updatedatatable(language, "auditdistricttable");
    }
    ...
    $("#translate").change(function() {
        ...
        updateValidationMessages(getLanguage('Y'), 'auditdistrictform');
    ...
    jsonLoadedPromise.then(() => {
        ...
        var validator = $("#auditdistrictform").validate({
            rules: {
                deptcode: {
                    required: true,
                },
        ...
        $("#buttonaction").on("click", function(event) {
            ...
            if ($("#auditdistrictform").valid()) {
                ...
                var formData = $('#auditdistrictform').serializeArray();
                ...
                $.ajax({
                    url: "{{ route('auditdistrict.auditdistrict_insertupdate') }}", // URL where the form data will be posted
                    type: 'POST',
                    data: formData,
    ...
    function groupForm(auditdis) {
        $('#display_error').hide();
        // change_button_as_update('groupform', 'action', 'buttonaction', 'display_error', '', '');
        // $('#catcode').val(mainobjection.catcode);
        $('#auditdistename').val(auditdis.auditdistename);
        $('#auditdisttname').val(auditdis.auditdisttname);
        $('#auditdistsname').val(auditdis.auditdistsname);

        $('#auditdistid').val(auditdis.encrypted_auditdistid);
        //alert(id);
        populateStatusFlag(auditdis.statusflag);
        $('#deptcode').val(auditdis.auditdeptcode).trigger('change');
                ...
                success: function(response) {
                    if (response.success) {
                        if (response.data && response.data.length > 0) {
                            changeButtonAction('auditdistrictform', 'action', 'buttonaction',
    ...
    function reset_form() {
        ...
        //$('#auditdistrictform').validate().resetForm();
        if (sessiondeptcode && sessiondeptcode.trim() !== '') {

            $('#auditdistename,#auditdisttname', '#auditdistsname', '#auditdistid').val();
        ...
        // alert();
        // $('#deptcode').val(null).trigger('change');
        changeButtonAction('auditdistrictform', 'action', 'buttonaction', 'display_error', '',
            @json($savebtn),
